...
var marshaltest = []struct {
	...
	"\x1f\x00\x00\x00\x03Inner\x00\x13\x00\x00\x00\x02Val\x00\x05\x00\x00\x00test\x00\x00\x00",
}, {
...
func TestStructTags(t *testing.T) {
	...
	expected := "\x1b\x00\x00\x00\x02b\x00\x04\x00\x00\x00bar\x00\x12z\x009\x05\x00\x00\x00\x00\x00\x00\x00"
	if expected != string(b) {
		t.Errorf("bad marshalling. got %q\nwant: \"%q\"", b, expected)
	}

	foo2 := Foo{}

	if err = Unmarshal(b, &foo2); err != nil {
		t.Error(err)
	}
